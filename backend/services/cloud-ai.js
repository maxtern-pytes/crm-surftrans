...
async function callCloudAI(prompt, systemPrompt = '', maxRetries = 2) {
  ...
  if (!TOGETHER_API_KEY) {
    throw new Error('TOGETHER_API_KEY not configured. Set it in your environment variables.');
  }
...
async function analyzeClientProspects(targetParams, existingClients = []) {
  const systemPrompt = `You are a US freight brokerage business development expert with knowledge of major companies across industries. Identify high-potential shipper prospects based on target parameters. Always respond with valid JSON only.`;
...
Generate 5-8 REAL high-potential prospect companies that would need freight brokerage services. Use actual company names from your knowledge base.
...
async function recommendLoads(agentPerformance, marketData = {}) {
  const systemPrompt = `You are a freight load optimization expert with knowledge of current market trends. Recommend profitable load opportunities. Always respond with valid JSON only.`;
...
module.exports = {
  ...
  clearCache,
  callCloudAI,
  callOllama: callCloudAI  // Alias for compatibility
};
